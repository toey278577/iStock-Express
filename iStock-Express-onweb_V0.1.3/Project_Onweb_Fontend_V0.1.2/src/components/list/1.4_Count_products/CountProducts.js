...
function CountProducts() {
  ...
  const handleSaveItem = async () => {
    try {
      ...
      await fetch(`http://localhost:5000/api/products/${editingItemData.productCode}`, {
        method: "PUT",
  ...
  const handleDeleteItem = async (index) => {
    try {
      ...
      await fetch(`http://localhost:5000/api/products/${productCode}`, { method: "DELETE" });
  ...
  return (
    <div className="container-wms">
      <div className="text-center mb-4">
        <h1 className="text-dark fw-bold">ระบบตรวจนับสินค้า</h1>
        <p className="text-muted">ระบบจัดการสินค้าคงคลัง</p>
      </div>

      {/* ฟอร์มเพิ่มสินค้าใหม่ */}
      <div className="card shadow-sm mb-4">
        <div className="card-header btn-gradient-red text-white">เพิ่มสินค้าใหม่</div>
        <div className="card-body">
          <form onSubmit={(e) => e.preventDefault()}>
              ...
              {columnNames.map((column) => (
                <div className="col-md-4" key={column.key}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewItemData({ ...newItemData, [column.key]: e.target.value })
                    }
                  />
                </div>
              ))}
            ...
            <div className="mt-4 text-center">
              <button className="btn btn-custom shadow-lg" onClick={handleAddNewItem}>
